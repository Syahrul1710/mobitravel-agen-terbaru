...
@section('title', 'Verifikasi Agen')
@section('menu-agents', 'active')

@section('content')
<div class="content">
    <div class="header">
        <h1>Manajemen Agen</h1>
    ...
    <div class="tabs">
        ...
        <a href="{{ route('admin.agents.index', ['status' => 'active']) }}" 
           class="tab {{ $status == 'active' ? 'active' : '' }}">Aktif</a>
        <a href="{{ route('admin.agents.index', ['status' => 'suspended']) }}" 
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="{{ route('admin.agents.show', $agent->id) }}" class="btn btn-primary btn-sm">Detail</a>
                    
                    @if($agent->status == 'pending')
                        <button onclick="showVerifyModal({{ $agent->id }}, '{{ $agent->agency_name }}')" 
                                class="btn btn-primary btn-sm">Setujui</button>
                        <button onclick="showRejectModal({{ $agent->id }}, '{{ $agent->agency_name }}')" 
                    ...
                    <button onclick="deleteAgent({{ $agent->id }}, '{{ $agent->agency_name }}')" 
                            class="btn btn-danger btn-sm">Hapus</button>
                </td>
            </tr>
            @empty
                <tr>
                    <td colspan="7" style="text-align: center;">Tidak ada data agen</td>
                </tr>
            @endforelse
        </tbody>
    </table>
...
<div id="verifyModal" class="modal">
    <div class="modal-content">
        <h3>Setujui Agen</h3>
        <p>Yakin ingin menyetujui agen <strong id="verifyName"></strong>?</p>
...
<div id="rejectModal" class="modal">
    <div class="modal-content">
        <h3>Tolak Agen</h3>
        <p>Agen: <strong id="rejectName"></strong></p>
...
<script>
    function showVerifyModal(id, name) {
        ...
        document.getElementById('verifyForm').action = '/admin/agents/' + id + '/verify';
        document.getElementById('verifyModal').style.display = 'flex';
    }
    
    function showRejectModal(id, name) {
        document.getElementById('rejectName').innerText = name;
        document.getElementById('rejectForm').action = '/admin/agents/' + id + '/reject';
    ...
    function suspendAgent(id) {
        if(confirm('Yakin ingin menangguhkan agen ini?')) {
            ...
            form.method = 'POST';
            form.action = '/admin/agents/' + id + '/suspend';
            form.innerHTML = '@csrf';
    ...
    function activateAgent(id) {
        if(confirm('Yakin ingin mengaktifkan kembali agen ini?')) {
            let form = document.createElement('form');
            form.method = 'POST';
            form.action = '/admin/agents/' + id + '/activate';
            form.innerHTML = '@csrf';
            document.body.appendChild(form);
            form.submit();
        }
    ...
    function deleteAgent(id, name) {
        if(confirm('Yakin ingin menghapus agen "' + name + '" secara permanen?')) {
            let form = document.createElement('form');
            form.method = 'POST';
